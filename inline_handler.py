...
def inline_query(update, context):
    ...
    try:
        ...
        if is_bookmark_query:
            if not bookmarks:
                ...
                for idx, b in enumerate(bookmarks[:50]):
                    ...
                    elif b_banner and isinstance(b_banner, str) and (b_banner.startswith("http://") or b_banner.startswith("https://")):
                        results.append(
                            InlineQueryResultPhoto(
                                id=f"bm_{b_cid}_{b_ch}_{idx}",
                                photo_url=b_banner,
                                thumbnail_url=b_banner,
                                title=f"📌 {b_manga_name} — Ch. {b_ch}",
                                description=f"Continue reading from Chapter {b_ch}",
                                caption=caption,
                                parse_mode="HTML",
        ...
        for idx, manga in enumerate(page_items):
            ...
            # 1. High-Speed Telegram Native Photo Message (Real covers + full photo poster in chat)
            ...
            # 3. Clean Interactive Article Card (Fallback if no photo)
            else:
                results.append(
                    InlineQueryResultArticle(
                        id=result_id,
